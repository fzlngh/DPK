...
import { useBeatPoints } from './useBeatPoints';
...
export default function BeatBangGame() {
  ...
  function handleTopupSuccess(pkg: TopupPackage, orderId: string, bonusBP?: number) {
    const total = pkg.points + pkg.bonus + (bonusBP ?? 0);
    creditPoints(pkg, orderId, bonusBP ?? 0);
    showNotif(`✅ +${total} BEATPOINTS DITAMBAHKAN!`);
  }
