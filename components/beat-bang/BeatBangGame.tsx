  ...
  while (t < duration) {
    for (let lane = 0; lane < 4; lane++) {
      if (Math.random() < density / 4)
        notes.push({ time: t + Math.random() * 0.05, lane });
    }
    if (difficulty === "hard" && Math.random() < 0.3)
      notes.push({
        time: t + beatInterval / 2 + Math.random() * 0.03,
...
function buildSongs(): Song[] {
  return [
    // ── FREE SONGS ──
    {
      ...
      difficulty: "medium",
      color: "#4FC3F7",
      pattern: generatePattern(128, 60, "medium"),
    },
    {
      id: "demo2",
      name: "Neon Rush",
      ...
      difficulty: "hard",
      color: "#FF6B6B",
      pattern: generatePattern(160, 60, "hard"),
    },
    {
      id: "demo3",
      name: "Chill Vibes",
      ...
      difficulty: "easy",
      color: "#81C784",
      pattern: generatePattern(90, 60, "easy"),
    },
    {
      id: "demo4",
      name: "Jazz Hands",
      ...
      difficulty: "medium",
      color: "#CE93D8",
      pattern: generatePattern(110, 60, "medium"),
    },
    {
      id: "demo5",
      name: "Rock Solid",
      ...
      difficulty: "hard",
      color: "#FFCC02",
      pattern: generatePattern(140, 60, "hard"),
    },
    // ── PREMIUM SONGS (butuh BeatPoints) ──
    {
      id: "premium1",
      name: "Galaxy Overdrive",
      artist: "StarForce",
      emoji: "🌌",
      genre: "EDM",
      bpm: 175,
      difficulty: "hard",
      color: "#AA00FF",
      pattern: generatePattern(175, 75, "hard"),
      premium: true,
      price: 20,
    },
    {
      ...
      difficulty: "medium",
      color: "#FF69B4",
      pattern: generatePattern(148, 75, "medium"),
      premium: true,
      price: 30,
    },
    {
      ...
      difficulty: "hard",
      color: "#FF4500",
      pattern: generatePattern(200, 75, "hard"),
      premium: true,
      price: 50,
    },
    // ── CUSTOM SLOTS ──
    {
      id: "custom1",
      name: "Your Music Here",
      artist: "Upload MP3/WAV",
      emoji: "📁",
      genre: "CUSTOM",
      bpm: 120,
      difficulty: "medium",
...
export default function BeatBangGame() {
  ...
  const gs = useRef({
    ...
    pauseStart: 0,
    spawnedNotes: 0,
    activeNotes: [] as NoteEvent[],
    activeSong: songsRef.current[0],
    lanes: 4 as 2 | 4,
    noteSkin: "circle" as NoteSkin,
    autoplay: false,
    volume: 0.7,
    speed: 1.0,
  ...
  useEffect(() => {
    ...
    const lb = ls.getJSON("bb_lb", []) as LeaderboardEntry[];
  ...
  function resizeCanvas() {
    ...
    if (w) {
      w.width = w.parentElement?.clientWidth || window.innerWidth;
      w.height = 56;
    }
  }
  ...
  function renderIdle() {
    ...
    for (let i = 0; i < lanes; i++) {
      ...
      ctx.beginPath();
      ctx.arc(x, hitY, 26, 0, Math.PI * 2);
      ctx.strokeStyle = LANE_COLORS[i] + "50";
      ctx.lineWidth = 8;
      ctx.stroke();
  ...
  function initAudio() {
    ...
    gain.gain.value = gs.current.volume;
    ...
    for (let ch = 0; ch < 2; ch++) {
      ...
      for (let i = 0; i < sr * duration; i++) {
        ...
        if (ph < 0.05)
          s +=
            0.6 *
            Math.sin(2 * Math.PI * 150 * Math.exp(-ph * 40) * ph) *
            Math.exp(-ph * 25);
        if (bt % 2 === 1 && ph < 0.04)
          s += 0.4 * (Math.random() * 2 - 1) * Math.exp(-ph * 30);
        const hp = (i % (bi / 2)) / (bi / 2);
        if (hp < 0.02) s += 0.2 * (Math.random() * 2 - 1) * Math.exp(-hp * 60);
        const bn = 60 + (bt % 4) * 7,
          bf = 440 * Math.pow(2, (bn - 69) / 12);
        s += 0.15 * Math.sin(2 * Math.PI * bf * t) * (1 - ph * 0.5);
        const mn = 72 + [0, 4, 7, 12, 7, 4, 2, 0][bt % 8],
          mf = 440 * Math.pow(2, (mn - 69) / 12);
        if (bt % 2 === 0 && ph < 0.3)
          s += 0.12 * Math.sin(2 * Math.PI * mf * t) * Math.exp(-ph * 3);
        d[i] = Math.tanh(s * 1.5);
      }
    }
    return buf;
  ...
  function playAudio(offset = 0) {
    ...
    src.buffer = buf;
    src.playbackRate.value = gs.current.speed;
    src.connect(analyserRef.current!);
    src.start(0, offset);
    audioStartRef.current = audioCtxRef.current.currentTime - offset;
    sourceRef.current = src;
    src.onended = () => {
      if (gs.current.isPlaying && !gs.current.isPaused) endGame();
    };
  }
  ...
  function updateHUD() {
    ...
    setLife(s.life);
    const a =
      s.hitNotes + s.missHits > 0
        ? Math.round((s.hitNotes / (s.hitNotes + s.missHits)) * 100)
        : 100;
  ...
  function renderFrame() {
    ...
    for (let i = 0; i < lanes; i++) {
      ...
      g.addColorStop(0.5, LANE_COLORS[i] + "0A");
    ...
    for (let i = 0; i < lanes; i++) {
      ...
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(x, hitY, 22, 0, Math.PI * 2);
      ctx.fillStyle = LANE_COLORS[i] + "25";
      ctx.fill();
      ctx.strokeStyle = "#1a1a1a";
      ctx.lineWidth = 2;
      ctx.stroke();
    ...
    for (const note of s.activeNotes) {
      ...
      const tu = note.time - now;
      if (tu > look || tu < -0.5) continue;
      const prog = 1 - tu / look,
        ny = prog * hitY,
        nx = (W / lanes) * (note.lane % lanes) + W / lanes / 2;
      if (tu < 0.3 && tu > -0.15) {
        const int = 1 - Math.abs(tu) / 0.3;
        ctx.beginPath();
        ctx.arc(nx, ny, 30 * int + 22, 0, Math.PI * 2);
        ctx.fillStyle =
          LANE_COLORS[note.lane % lanes] +
          Math.floor(int * 80)
            .toString(16)
            .padStart(2, "0");
        ctx.fill();
    ...
    if (bp > 0) {
      ctx.fillStyle = `rgba(255,224,0,${bp * 0.12})`;
      ctx.fillRect(0, 0, W, H);
    }
    ...
    if (s.autoplay) {
      ...
      ctx.textAlign = "center";
      ctx.fillText("🤖 AUTO-PLAY (DEMO)", W / 2, 28);
    }
  }
  ...
  function renderWaveform() {
    ...
    if (!analyserRef.current || !gs.current.isPlaying) {
      renderWaveIdle();
      return;
    }
  ...
  function gameLoop() {
    ...
    while (
      s.spawnedNotes < pat.length &&
      pat[s.spawnedNotes].time <= now + 2.5
    ) {
    ...
    for (const n of s.activeNotes)
      if (!n.hit && !n.missed && now > n.time + 0.2) {
        n.missed = true;
        onMiss();
      }
    ...
    if (s.autoplay)
      for (const n of s.activeNotes)
        if (!n.hit && !n.missed && Math.abs(now - n.time) < 0.06) {
          n.hit = true;
          onHit("perfect", n.lane % s.lanes);
        }

    if (s.activeNotes.length > 200)
      s.activeNotes = s.activeNotes.filter((n) => n.time > now - 1);

    const dur = s.activeSong.audioBuffer
      ? s.activeSong.audioBuffer.duration
      : 65;
    setProgressPct(Math.min(100, (now / dur) * 100));
    const ms = Math.floor(now / 60),
      ss = Math.floor(now % 60)
        .toString()
        .padStart(2, "0");
    const tm = Math.floor(dur / 60),
      ts = Math.floor(dur % 60)
        .toString()
        .padStart(2, "0");
  ...
  function onHit(type: string, lane: number) {
    const s = gs.current,
      pts: Record<string, number> = { perfect: 300, great: 150, good: 80 };
    s.score += Math.round(pts[type] * (1 + Math.floor(s.combo / 10) * 0.5));
    s.combo++;
    s.maxCombo = Math.max(s.maxCombo, s.combo);
    s.hitNotes++;
    if (type === "perfect") s.perfectHits++;
    else if (type === "great") s.greatHits++;
    else s.goodHits++;
    flashLane(lane, type);
    updateHUD();
  }
  ...
  function onMiss() {
    ...
    if (el) {
      el.classList.add("bb-shake");
      setTimeout(() => el.classList.remove("bb-shake"), 300);
    }
  }

  function tapLane(lane: number) {
    const s = gs.current;
    if (!s.isPlaying || s.isPaused || lane >= s.lanes) return;
    const now = getAudioTime();
    let best: NoteEvent | null = null,
      bd = Infinity;
    for (const n of s.activeNotes) {
      if (n.hit || n.missed || n.lane % s.lanes !== lane) continue;
      const d = Math.abs(now - n.time);
      if (d < bd) {
        bd = d;
        best = n;
      }
    ...
    if (!best || bd > 0.25) {
      spawnHitText("MISS!", lane, "miss");
      return;
    }
    best.hit = true;
    const ht = bd < 0.06 ? "perfect" : bd < 0.12 ? "great" : "good";
    onHit(ht, lane);
    spawnHitText(ht.toUpperCase() + "!", lane, ht);
    spawnParticles(lane);
  }
  ...
  function handlePlay() {
    if (!gs.current.isPlaying) startCountdown(doStartGame);
  }

  function handlePlayPause() {
    if (!gs.current.isPlaying) {
      handlePlay();
      return;
    }
    const s = gs.current;
    if (s.isPaused) {
      ...
      playAudio(getAudioTime());
      setIsPaused(false);
      animFrameRef.current = requestAnimationFrame(gameLoop);
    } else {
      s.isPaused = true;
      s.pauseStart = audioCtxRef.current!.currentTime;
      if (sourceRef.current) {
        try {
          sourceRef.current.stop();
        } catch (_) {}
  ...
  function endGame() {
    ...
    const best = parseInt(ls.get("bb_best", "0"));
    if (finalScore > best) {
      ls.set("bb_best", String(finalScore));
      setBestScore(finalScore);
    }
    const totalHits = s.perfectHits + s.greatHits + s.goodHits;
    const accV =
      s.totalNotes > 0 ? Math.round((totalHits / s.totalNotes) * 100) : 0;
    const grade = getGrade(accV);
    if (!s.autoplay) {
      const lb: LeaderboardEntry[] = ls.getJSON(
        "bb_lb",
        [],
      ) as LeaderboardEntry[];
      lb.push({
  ...
  async function handleAudioUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const slot = songsRef.current.find(
        (s) => s.isCustomSlot && !s.audioBuffer,
      );
      if (slot) {
        slot.audioBuffer = decoded;
        slot.name = file.name.replace(/\.[^.]+$/, "");
        slot.artist = "Your Upload";
        slot.emoji = "🎵";
        slot.isCustomSlot = false;
        slot.duration = decoded.duration;
        slot.pattern = generatePattern(
          120,
          decoded.duration,
          gs.current.difficulty,
        );
        setSongs([...songsRef.current]);
        showNotif("🎵 MUSIC LOADED!");
      } else showNotif("⚠️ NO MORE SLOTS!");
    } catch {
      showNotif("⚠️ COULD NOT LOAD");
    }
    e.target.value = "";
  }
  ...
  function handleSaveSettings(ns: GameSettings) {
    setSettings(ns);
    ls.set("bb_name", ns.playerName);
    setSettingsOpen(false);
    showNotif("⚙️ SETTINGS SAVED!");
    songsRef.current.forEach((s) => {
      if (!s.audioBuffer) s.pattern = generatePattern(s.bpm, 65, ns.difficulty);
    });
    setSongs([...songsRef.current]);
  }
  ...
  const activeSong =
    songsRef.current.find((s) => s.id === activeSongId) || songsRef.current[0];
  ...
  `}</style>
      ...
      <div className="bb-wrap">
        ...
        <div className="bb-hud">
          <div className="bb-logo">
            BEAT<span style={{ color: "#FF2D2D" }}>BANG!</span>
          </div>
          ...
          <div className="bb-hud-stats">
            {[
              ...
              {
                label: "🔥 BEST",
                val: bestScore.toLocaleString(),
                color: "#FFE000",
              },
            ].map((s) => (
              <div key={s.label} className="bb-stat">
                <div className="bb-stat-label">{s.label}</div>
                <div className="bb-stat-val" style={{ color: s.color }}>
                  {s.val}
                </div>
              </div>
            ))}
          ...
          <div className="bb-bottom-hud">
            ...
            <div className="bb-hearts">
              {Array.from({ length: 5 }).map((_, i) => (
                <span key={i} className={`bb-heart${i >= life ? " lost" : ""}`}>
                  ❤️
                </span>
              ))}
            </div>
      ...
      <div
        className={`bb-sidebar-backdrop${sidebarOpen ? " open" : ""}`}
      ...
      <div className={`bb-sidebar${sidebarOpen ? " open" : ""}`}>
        ...
        <div className="bb-sidebar-tabs">
          <button
            className={`bb-sidebar-tab${!lbOpen ? " active" : ""}`}
        ...
        <div className="bb-sidebar-body">
          {!lbOpen ? (
            <>
              ...
              {songs
                .filter((s) => !s.premium && !s.isCustomSlot)
                .map((song) => (
                  <div
                    key={song.id}
                      ...
                      >
                        ...
                        <span
                          style={{
                            ...
                            padding: "1px 6px",
                            border: "2px solid #1a1a1a",
                            background: diffColor[song.difficulty],
                            color: "#fff",
                            borderRadius: 2,
                          }}
                        >
                ...
                onDrop={(e) => {
                  ...
                  if (f) {
                    ...
                    if (fi) {
                      ...
                      handleAudioUpload({ target: fi } as any);
                    }
                  }
                }}
              >
                ...
                <div style={{ fontSize: 11, color: "#666", marginTop: 2 }}>
                  Click or drag & drop MP3/WAV/OGG
                </div>
              </div>
              <input
                ref={fileInputRef}
      ...
      {purchaseModal && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setPurchaseModal(null);
          }}
        >
          <div
            style={{
            ...
            <div style={{ padding: 24 }}>
              ...
              >
                ...
                <div>
                  ...
                  >
                    {purchaseModal.artist} • {purchaseModal.bpm} BPM
                  </div>
                  <div
                    style={{
                      fontFamily: "Bangers,cursive",
                  ...
                  >
                    {purchaseModal.difficulty.toUpperCase()}
                  </div>
                </div>
              </div>
              ...
              {beatPoints < (purchaseModal.price || 0) && (
                ...
                >
                  ❌ BeatPoints tidak cukup! Kurang{" "}
                  {(purchaseModal.price || 0) - beatPoints} BP.
                </div>
              )}
